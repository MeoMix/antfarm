...
function World({ elements, ants, surfaceLevel, antColor }: Props) {
  ...
  return (
    <>
      ...
      <Air x={0} y={0} width={elements.filter(({ location }) => location.y === 0).length} height={surfaceLevel + 1} />
      <Tunnel x={0} y={surfaceLevel + 1} width={elements.filter(({ location }) => location.y === 0).length} height={elements.length - surfaceLevel - 1} />
